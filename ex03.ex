...
defmodule Ex03 do
  ...
  def odd_even([_ | t]),                         do: [:odd | odd_even(t)]
  def odd_even([]),                              do: []
  ...
  def list_contains([h | _], val) when (h == val), do: true
  def list_contains([_ | t], val),                 do: list_contains(t, val)
  ...
  def list_equal([a | t1], [a | t2]), do: list_equal(t1, t2)
