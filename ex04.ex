...
defmodule Ex04 do
  ...
  def min(list) do
    reduce(list, &(if &1 < &2 do &1 else &2 end))
  end
  ...
  def even_odd(list) do
    reduce(reverse(list), {[],[]}, fn
      (item, {evens, odds}) when Integer.is_even(item) -> {[item | evens], odds}
      (item, {evens, odds}) when Integer.is_odd(item) -> {evens, [item | odds]}
    end)
  end
